...
from settings import count_room, seve_program_settings, load_program_settings, get_all_unique_date_from_base, unplan_all_meetings_in_future
...
# works_minutes = ["00", "10", "20", "30", "40", "50"]  # шаг планирования в минутах


# обрабатывает действие кнопки сохранения заявки на встречу и вызывает фцнкцию записи в базу
def planing_this_meet():
...
def get_and_save_program_settings():
    ...
    try:
        ...
        unplan_all_meetings_in_future(base_name, DATE_NOW)
    except:
        pass
...
def create_table_meetingrooms_as_text():
    text = "Комната\t\t  Мест\t\tОпция1\t\tОпция2\n"
    ...
    with sq.connect(base_name) as con:
        ...
        if list_meet_one_date:
            ...
            for i in range(len(list_meet_one_date)):
                ...
                planed = list_meet_one_date[i][7]
                if planed == 0:
                    planed = '---'
                text_meet_lb2 += f"{num_meet}\t{date_meet}\t{time_start}\t\t{time_end}\t\t{persons_of_meeting}\t\t{planed}\n"
                list_num_meet.append(num_meet)

        cb2_f3_date.configure(values=list_num_meet)
...
def get_meeting_list_on_this_date(this_date):
    ...
    with sq.connect(base_name) as con:
        cursor = con.cursor()
        text = f"SELECT * FROM Meetings WHERE datemeet = {date_meet} and planned = 0"
        cursor.execute(text)
        list_meet_one_date = cursor.fetchall()
...
def make_meeting_as_unplaned():
    number = cb2_f3_date.get()
    with sq.connect(base_name) as con:
        cursor = con.cursor()
        text = f"UPDATE Meetings SET planned = 0 WHERE _id = {number}"
        cursor.execute(text)


# создает базовый план на дату с учетом уже запланированных встреч
def make_base_plan_for_all_miteeng_rooms(date = str(DATE_NOW.date())):
    global plans_all_miteeng_rooms
    plans_all_miteeng_rooms = []

    # создаем рыбу плана на существующее кол-во комнат
    for i in range(len(properties_of_meeting_rooms)):
        plans_all_miteeng_rooms.append([])

    # ищем в базе запланированные встречи на эту дату
    with sq.connect(base_name) as con:
        cursor = con.cursor()
        text = f"SELECT * FROM Meetings  WHERE datemeet = {date} and planned != 0"
        cursor.execute(text)
        list_all_planed_meet_for_date = cursor.fetchall()
        # print(f"list_all_planed_meet_for_date {list_all_planed_meet_for_date}")

    # заполняем рыбу найдеными встречами
    for meet in list_all_planed_meet_for_date:
        meet_num = meet[0]
        meet_start = meet[2]
        meet_end = meet[3]
        meet_pers = meet[4]
...
def make_list_for_table_meetings_plan_on_date(list_meetings):
    ...
    for room in range(len(list_meetings)):
        for meeting in range(len(list_meetings[room])):
            meet = (room + 1, list_meetings[room][meeting][0], list_meetings[room][meeting][1], list_meetings[room][meeting][2], list_meetings[room][meeting][3])
...
notebook.add(frame4, text=" Планирование ")
...
btn = Button(frame1, text="Запланировать", command=planing_this_meet)
...
lb2_f2 = Label(frame2, text="Комната                   Мест                  Опция1                   Опция2", anchor="w")
...
"""Закладка №3. Встречи"""
lbl1_f3 = Label(frame3, text="На дату", anchor="w")
lbl1_f3.place(x=20, y=15, height=25, width=80)

cb1_f3_date = Combobox(frame3, values=list_uniq_date, state="readonly")
cb1_f3_date.place(x=80, y=15, height=25, width=90)

lbl2_f3_text = "  #                 дата                   начало                окончание           кол-во                 комната №"
lbl2_f3 = Label(frame3, text=lbl2_f3_text, anchor="w")
lbl2_f3.place(x=20, y=45, height=25, width=490)

lbl3_f3 = Label(frame3, text="нет встреч", anchor="center", background="#FFFFFF")
lbl3_f3.place(x=20, y=70, height=250, width=520)

btn1_f3 = Button(frame3, text="Проверить", command=get_from_base_meeting_for_a_day)
btn1_f3.place(x=180, y=15, width=80, height=25)

lbl4_f3 = Label(frame3, text="Встречу", anchor="w")
lbl4_f3.place(x=345, y=15, height=25, width=80)

cb2_f3_date = Combobox(frame3, values="", state="readonly")
cb2_f3_date.place(x=400, y=15, height=25, width=50)

btn2_f3 = Button(frame3, text="Открепить", command=make_meeting_as_unplaned)
btn2_f3.place(x=460, y=15, width=80, height=25)

"""Закладка №4. Планирование"""
lbl1_f4 = Label(frame4, text="Дата", anchor="w")
lbl1_f4.place(x=20, y=15, height=25, width=80)

cb3_f4_date = Combobox(frame4, values=list_uniq_date, state="readonly")
...
# настраиваем столбцы
tree.column("#1", stretch=NO, width=70, anchor='center')
tree.column("#2", stretch=NO, width=70, anchor='center')
tree.column("#3", stretch=NO, width=70, anchor='center')
tree.column("#4", stretch=NO, width=70, anchor='center')
tree.column("#5", stretch=NO, width=70, anchor='center')
